...
const createYearlyGraphData = (data, labels, aggregationMethod = "average") => {
  ...
  const monthNames = uniqueMonths.map((monthNumber) =>
...
const CustomBarChart = ({ data, selectedDate, aggregationMethod }) => {
  ...
  const yearlyGraphData = createYearlyGraphData(
    data.data,
    dataLabels,
    aggregationMethod
  );
  ...
  return (
    <Box sx={{ width: currentGraphWidth, minWidth: "100%", height: "320px" }}>
