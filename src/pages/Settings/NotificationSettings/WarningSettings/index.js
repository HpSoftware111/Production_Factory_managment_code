...
const WarningSettings = () => {
  ...
  const handleEdit = (row) => {
    ...
    navigate("/settings/notification-settings/warningSettings/edit", {
      state: { id: row["EmployeeID"] },
    });
  };

  const handleDelete = (row) => {
    navigate("/settings/notification-settings/warningSettings/delete", {
      state: { id: row["EmployeeID"] },
    });
  };
  ...
  useEffect(() => {
    if (rowsPerPage) {
      axios
        .get(
          `/notifications?page=${currentPage}&keyword=${searchTerm}&size=${rowsPerPage}&Notification_Type=Warning` // Add Notification_Type to the query
        )
        .then((res) => {
          console.log("res:", res);
