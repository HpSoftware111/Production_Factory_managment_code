...
const Vendors = () => {
  ...
  const handleEdit = (row) => {
    navigate(`/vendor/manage-vendors/edit`, {
      state: { id: row["VendorID"] },
    });
  };

  const handleDelete = (row) => {
    navigate(`/vendor/manage-vendors/delete`, {
      state: { id: row["VendorID"] },
    });
  };
  ...
  const tableHeaders = [
    ...
    { id: "Vendor_Main_Phone", label: breakLabelText("Vendor Phone Number") },
    { id: "Vendor_Fax_Phone", label: breakLabelText("Vendor Fax Number") },
    { id: "Contact_Name", label: breakLabelText("Primary Contact Name") },
    { id: "Contact_Main_Phone", label: breakLabelText("Contact Phone Number") },
    { id: "Contact_Email", label: breakLabelText("Contact Email Address") },
  ...
  useEffect(() => {
    const calculateRowsPerPage = () => {
      const availableHeight = window.innerHeight - 367;
      const rowHeight = 72.5;
      const rowGap = 20;
  ...
  useEffect(() => {
    if (rowsPerPage) {
      axios
        .get(
          `/vendors?page=${currentPage}&keyword=${searchTerm}&size=${rowsPerPage}`
        )
        .then((res) => {
